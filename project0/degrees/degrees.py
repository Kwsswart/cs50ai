...
def shortest_path(source, target):
    ...
    # Set-up frontier and starting point
    ...
    while True:
        ...
        for movie, person in neighbors:
            ...
            # Check if we have checked this person already
            if person not in explored and not frontier.contains_state(person):
                # Store and check the returned neighbor and then check if target. If not add to frontier
                new_node = Node(state=person, parent=node, action=movie)
                if new_node.state == target:
                    # If is target trace back entire route and return
                    route = []
                    node = new_node
